...
const targetDaysMap = `                 {days.map((d, i) => {
                    ...
                    if (selectedDay === i) {
                       ...
                       dayClass = 'bg-[#00E676] border-text-main text-text-main neo-card shadow-[4px_4px_0_0_var(--color-text-main)] hover:-translate-y-1';
                    } else {
                       dayClass = 'bg-[#FF3D00] border-text-main text-text-main neo-card shadow-[4px_4px_0_0_var(--color-text-main)] hover:-translate-y-1';
                    }
...
const replDaysMap = `                 {days.map((d, i) => {
                    const assignedMenu = activeMenus.find(a => a.dateId === d.id);
                    const hasMenuAssigned = !!assignedMenu && assignedMenu.menuId !== '';
                    let dayClass = '';
                    let dotClass = '';
...
if (code.includes(targetDaysMap)) {
    code = code.replace(targetDaysMap, replDaysMap);
    fs.writeFileSync('src/components/PlanView.tsx', code, 'utf-8');
    console.log('Replaced exact successfully');
} else {
    console.log('Not found exactly. Doing manual replacement.');
}
