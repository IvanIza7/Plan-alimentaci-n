...
export default function InventoryView() {
  ...
  const handleUpdateAmount = async () => {
     if (selectedItem) {
        await updateInventoryItem(selectedItem.id, { amount: quickAmount });
        setSelectedItem(null);
     }
  };
  
  const handleAddToShopping = (item: InventoryItem) => {
     addShoppingItem({
        name: item.name,
        qty: `${item.lowThreshold} ${item.unit}`,
        category: item.category.split(',')[0].toUpperCase(),
        icon: item.icon,
        checked: false
     });
     setSelectedItem(null);
      ...
      {selectedItem && (
         <div className="fixed inset-0 bg-text-main/20  z-[100] flex items-end sm:items-center justify-center p-0 sm:p-4 animate-in fade-in">
            <div className="bg-surface border-t-2 border-l-2 border-r-2 sm:border-b-2 border-text-main rounded-t-[32px] sm:rounded-[32px] p-6 w-full max-w-md neo-card shadow-[0_-8px_0_0_var(--color-text-main)] sm:shadow-[8px_8px_0_0_var(--color-text-main)] relative flex flex-col items-center">
               <button onClick={() => setSelectedItem(null)} className="absolute top-4 right-4 w-8 h-8 bg-background border-2 border-border-subtle rounded-full flex items-center justify-center text-text-secondary hover:text-text-main hover:border-text-main">
                  <X size={16} />
               </button>
               ...
               <div className="w-full bg-background border-2 border-border-subtle rounded-2xl p-4 mb-6">
               ...
               <div className="w-full flex gap-3">
                  <button onClick={() => { deleteInventoryItem(selectedItem.id); setSelectedItem(null); }} className="w-14 shrink-0 bg-red-100 text-red-500 border-2 border-red-500 rounded-xl font-black flex items-center justify-center">
                     <X size={20} />
                  </button>
                  <button onClick={handleUpdateAmount} className="flex-1 bg-primary-900 border-2 border-text-main text-surface font-black uppercase tracking-widest py-4 rounded-xl neo-btn hover:bg-primary-800 shadow-[4px_4px_0_0_var(--color-text-main)]">
                     Actualizar
                  </button>
               </div>
            </div>
      ...
      {showAddModal && (
         <div className="fixed inset-0 bg-text-main/20  z-[100] flex items-end sm:items-center justify-center p-0 sm:p-4 animate-in fade-in">
            <div className="bg-surface border-t-2 border-l-2 border-r-2 sm:border-b-2 border-text-main rounded-t-[32px] sm:rounded-[32px] p-6 w-full max-w-md neo-card shadow-[0_-8px_0_0_var(--color-text-main)] sm:shadow-[8px_8px_0_0_var(--color-text-main)] max-h-[90vh] overflow-y-auto relative">
               <div className="flex justify-between items-center mb-6 sticky top-0 bg-surface z-10 py-2 border-b-2 border-border-subtle pb-4">
               ...
               <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                     <div>
                        <label className="block text-[9px] font-black uppercase tracking-widest text-text-secondary mb-1">Cantidad Actual</label>
                        <input type="number" placeholder="0" value={addAmount} onChange={e => setAddAmount(e.target.value ? Number(e.target.value) : '')} className="w-full bg-background border-2 border-text-main rounded-xl p-3 font-bold text-sm focus:outline-none focus:border-primary-500" />
                     </div>
                     <div>
                        <label className="block text-[9px] font-black uppercase tracking-widest text-text-secondary mb-1">Unidad</label>
                        <select value={addUnit} onChange={e => setAddUnit(e.target.value)} className="w-full bg-background border-2 border-text-main rounded-xl p-3 font-bold text-sm focus:outline-none focus:border-primary-500">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                     <div>
                        <label className="block text-[9px] font-black uppercase tracking-widest text-text-secondary mb-1">Límite Bajo (Aviso)</label>
                        <input type="number" placeholder="Ej: 3" value={addThreshold} onChange={e => setAddThreshold(e.target.value ? Number(e.target.value) : '')} className="w-full bg-background border-2 border-text-main rounded-xl p-3 font-bold text-sm focus:outline-none focus:border-primary-500" />
                     </div>
                     <div>
                        <label className="block text-[9px] font-black uppercase tracking-widest text-text-secondary mb-1">Kcal (Aporte)</label>
                        <input type="number" placeholder="Ej: 52" value={addKcal} onChange={e => setAddKcal(e.target.value ? Number(e.target.value) : '')} className="w-full bg-background border-2 border-text-main rounded-xl p-3 font-bold text-sm focus:outline-none focus:border-primary-500" />
                     </div>
                  </div>
