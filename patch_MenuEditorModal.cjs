...
// 2. Add coverImage state
code = code.replace(
  "export default function MenuEditorModal({ menu, onClose, onSave }: MenuEditorProps) {",
...
// 3. Add UI for image selection and Delete button
const uiToInsert = `
          ...
          <div className="flex justify-between items-center mb-6">
            ...
            {onDelete && (
               <button onClick={onDelete} className="w-10 h-10 rounded-full bg-red-100 border-2 border-text-main flex items-center justify-center text-red-600 hover:bg-red-500 hover:text-white shadow-[2px_2px_0_0_var(--color-text-main)] transition-colors">
                  <Trash2 size={18} />
               </button>
            )}
          </div>
          ...
          <div className="mb-6">
             ...
             <input 
                ...
                className="w-full bg-background border-2 border-text-main rounded-xl p-3 font-bold text-sm shadow-[2px_2px_0_0_var(--color-text-main)] focus:outline-none focus:translate-y-1 focus:shadow-[0_0_0_0_var(--color-text-main)] transition-all"
             />
          </div>
...
code = code.replace(
  /<div className="mb-6">\s*<label className="block text-\[10px\] font-black uppercase tracking-widest text-text-secondary mb-2">Nombre del menú<\/label>\s*<input[^>]+>\s*<\/div>/,
  uiToInsert
);

// 4. Send coverImage onSave
code = code.replace(
  "onSave({ title, meals })",
  "onSave({ title, meals, coverImage })"
);

fs.writeFileSync('src/components/MenuEditorModal.tsx', code, 'utf-8');
console.log("Patched MenuEditorModal.tsx");
